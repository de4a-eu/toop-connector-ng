...
public final class EBMSUtils {
    ...
    /*
     * See http://docs.oasis-open.org/ebxml-msg/ebms/v3.0/profiles/AS4-profile/v1.0/
	...
	try {
	    final StreamSource stylesource = TransformSourceFactory
		    .create(new ClassPathResource("/receipt-generator.xslt"));
	...
	}
	{
		...
		eReason.appendElement(NS_SOAPENV, "Text").setAttribute(XMLConstants.XML_NS_URI, "lang", "en")
			.appendText(fm);
	    }
	}
	...
	try {
	    ...
	    final Element element = DOMReader
		    .readXMLDOM(MicroWriter.getNodeAsBytes(aDoc, new XMLWriterSettings().setNamespaceContext(aNSCtx)))
		    .getDocumentElement();
     ...
     *
     * @param message the soap message to be converted to a MEMessage. Cannot be
     *                null
	...
	if (errorElement != null) {
	    ...
	    final String severity = StringHelper.getNotNull(errorElement.getAttribute("severity"))
		    .toUpperCase(Locale.US);
